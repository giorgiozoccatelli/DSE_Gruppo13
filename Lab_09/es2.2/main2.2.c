...
int main(void)
{
  ...
  LL_GPIO_WriteOutputPort(GPIOA, LL_GPIO_ReadOutputPort(GPIOA) &~ ((1 << 5) | (1 << 6) | (1 << 7)));
  LL_GPIO_WriteOutputPort(GPIOB, LL_GPIO_ReadOutputPort(GPIOB) &~ (1 << 0));
...
void SystemClock_Config(void)
{
  ...
  LL_RCC_PLL_ConfigDomain_SYS(LL_RCC_PLLSOURCE_HSI, LL_RCC_PLLM_DIV_8, 84, LL_RCC_PLLP_DIV_4);
  LL_RCC_PLL_Enable();
